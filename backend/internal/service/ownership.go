...
// the resume itself so callers that need it don't have to fetch it twice.
// Every section-scoped service call goes through this before touching child
// rows, so ownership enforcement lives in exactly one place regardless of
// which adapter (REST today, MCP later) calls in.
